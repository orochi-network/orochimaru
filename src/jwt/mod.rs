...
impl JWT {
    pub fn new(secret_hex_string: &String) -> Self {
        JWT {
            secret_key: hex::decode(secret_hex_string.replace("0x", "").replace("0X", "")).unwrap(),
        }
    }
    ...
    pub fn decode_payload(json_web_token: &String) -> Option<JWTPayload> {
        ...
        if split_jwt.len() == 3 {
            let decoded_payload = base64_url::decode(&split_jwt[1]).unwrap();
            let regex_name = Regex::new(r#"^[a-zA-Z0-9\s]{3,40}$"#).expect("Unable to init Regex");
            let jwt_payload: JWTPayload = serde_json::from_slice(&decoded_payload).unwrap();
    ...
    pub fn verify(&self, json_web_token: &String) -> bool {
        ...
        if split_jwt.len() == 3 {
            ...
            mac.update(&base64_url::decode(&payload).unwrap());
            return match mac.verify_slice(&base64_url::decode(&signature).unwrap()) {
                Ok(_) => true,
                _ => false,
            };
        }
